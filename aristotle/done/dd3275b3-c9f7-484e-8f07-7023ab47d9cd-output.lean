...
This project request had uuid: dd3275b3-c9f7-484e-8f07-7023ab47d9cd
...
-- 4 of 7 proved by Aristotle (Harmonic); 3 remain sorry-stubbed.
import Mathlib
...
/-
Updating a list of lists at index `i` changes the sum of lengths. Specifically, the new sum plus the old length at `i` equals the old sum plus the new length. This formulation avoids natural number subtraction issues.
-/
...
-/
theorem CamelUp.setStack_eq_set (b : CamelUp.Board) (sq : Nat) (s : CamelUp.Stack) :
    (b.setStack sq s).toList = b.toList.set sq s := by
  unfold CamelUp.Board.setStack;
  -- By definition of `CamelUp.Board.setStack.setAt`, we can prove this by induction on the list `l`.
  induction' b.toList with l ih generalizing sq;
  · cases sq <;> aesop;
  · cases sq <;> simp_all +decide [ CamelUp.Board.setStack.setAt ]

/-
The sum of stack lengths on the board computed via `foldl` over indices is equal to the sum of lengths of the list of stacks.
-/
theorem CamelUp.board_sum_eq_foldl (b : CamelUp.Board) :
    (List.range b.size).foldl (fun acc i => acc + (b.getStack i).length) 0 = (b.toList.map List.length).sum := by
  -- The sum over the indices of the list of stacks is equivalent to the sum over the list of stacks directly.
...
-/
theorem CamelUp.sum_setStack_helper (b : CamelUp.Board) (sq : Nat) (s : CamelUp.Stack) (h : sq < CamelUp.numSquares) (hb : b.size = CamelUp.numSquares) :
...
theorem applyDie_preserves_totalCount
  ...
  · -- Apply the helper lemma to show that the total number of camels on the board remains the same after the move.
...
-/
def CamelUp.totalCamelCount (b : CamelUp.Board) (c : CamelUp.CamelColor) : Nat :=
  (List.range CamelUp.numSquares).foldl (fun acc i => acc + (CamelUp.Board.getStack b i).count c) 0

/-
If `findCamelOnBoard` finds a camel at square `sq`, then `sq` is a valid square index (less than `numSquares`).
-/
theorem CamelUp.findCamelOnBoard_bounds (b : CamelUp.Board) (c : CamelUp.CamelColor) (sq idx : Nat)
    (h : CamelUp.findCamelOnBoard b c = some (sq, idx)) : sq < CamelUp.numSquares := by
      -- By definition of `findCamelOnBoard.searchSquares`, if it returns `some (sq, idx)`, then `sq` must be a valid index in the list of squares.
      have h_sq_valid : sq ∈ List.range CamelUp.numSquares := by
        have h_search : ∀ (squares : List Nat), CamelUp.findCamelOnBoard.searchSquares b c squares = Option.some (sq, idx) → sq ∈ squares := by
          intros squares hsquares
          induction' squares with squares_sq squares_sq ih generalizing sq idx
          all_goals generalize_proofs at *;
          · cases hsquares;
          · unfold CamelUp.findCamelOnBoard.searchSquares at hsquares; aesop;
        exact h_search _ h
      generalize_proofs at *; (
      exact List.mem_range.mp h_sq_valid |> Nat.lt_of_lt_of_le <| by decide;)

/-
Retrieving the stack at the index where it was just set returns the new stack, provided the index is valid.
-/
theorem CamelUp.getStack_setStack_eq (b : CamelUp.Board) (sq : Nat) (s : CamelUp.Stack) (h : sq < b.size) :
    (b.setStack sq s).getStack sq = s := by
      -- Unfold `getStack` and `setStack`. `h` implies `sq` is a valid index for `b`, allowing the `getD` calls to simplify.
      simp only [Board.getStack, Board.setStack] at *; simp [h, Array.getD];
      -- Since `sq` is a valid index, `setAt` replaces the element at `sq` with `s`, and thus `getD` returns `s`.
      have h_setAt_sq : (CamelUp.Board.setStack.setAt s b.toList sq)[sq]?.getD [] = s := by
        have h_setAt_sq : ∀ (l : List CamelUp.Stack) (sq : Nat) (s : CamelUp.Stack), sq < l.length → (CamelUp.Board.setStack.setAt s l sq)[sq]?.getD [] = s := by
          intros l sq s h_sq_lt_l_len
          induction' l with l_head l_tail ih generalizing sq s
          all_goals generalize_proofs at *;
          · contradiction;
          · rcases sq with ( _ | sq ) <;> simp_all +arith +decide [ CamelUp.Board.setStack.setAt ];
        exact h_setAt_sq _ _ _ ( by simpa using h );
      aesop

/-
Setting a stack at a specific square does not change the stacks at other squares.
-/
theorem CamelUp.getStack_setStack_ne (b : CamelUp.Board) (sq i : Nat) (s : CamelUp.Stack) (h_ne : i ≠ sq) :
    (b.setStack sq s).getStack i = b.getStack i := by
      unfold CamelUp.Board.setStack CamelUp.Board.getStack;
      -- By definition of `setAt`, if `i ≠ sq`, then the element at index `i` in the modified list is the same as in the original list.
      have h_setAt : ∀ (l : List CamelUp.Stack) (s : CamelUp.Stack) (sq : Nat) (i : Nat), i ≠ sq → (CamelUp.Board.setStack.setAt s l sq).getD i [] = l.getD i [] := by
        intros l s sq i hi;
...
/-
Updating a stack at `sq` changes the total camel count by `new_stack_count - old_stack_count`. Expressed as an equality of sums to avoid subtraction issues.
-/
theorem CamelUp.totalCamelCount_setStack (b : CamelUp.Board) (sq : Nat) (s : CamelUp.Stack) (c : CamelUp.CamelColor)
    (h_size : b.size = CamelUp.numSquares) (h_sq : sq < CamelUp.numSquares) :
    CamelUp.totalCamelCount (b.setStack sq s) c + (b.getStack sq).count c =
    CamelUp.totalCamelCount b c + s.count c := by
      unfold CamelUp.CamelUp.totalCamelCount;
      -- We can split the sum into the term for `sq` and the terms for `i ≠ sq`.
...
theorem applyDie_preserves_camelCount
  ...
  -- If the camel is not found, then the board and finished list remain unchanged, so the total count is preserved.
  ...
  · obtain ⟨sq, idx, h_sq, h_idx⟩ : ∃ sq idx, findCamelOnBoard board outcome.camel = some (sq, idx) ∧ sq < numSquares := by
    ...
    · -- Apply the `totalCamelCount_setStack` theorem to show that the total count is preserved.
      ...
      have h_totalCamelCount_setStack : (List.foldl (fun acc i => acc + List.count c (CamelUp.Board.getStack ((board.setStack sq (List.take idx (board.getStack sq))).setStack (match mods[sq + outcome.value.toNat]?.getD Option.none with
        | Option.none => (sq + outcome.value.toNat, Option.none)
        | Option.some tile => (if 0 < tile.effect then sq + outcome.value.toNat + 1 else sq + outcome.value.toNat - 1, Option.some tile.owner)).1 ((board.setStack sq (List.take idx (board.getStack sq))).getStack (match mods[sq + outcome.value.toNat]?.getD Option.none with
        | Option.none => (sq + outcome.value.toNat, Option.none)
        | Option.some tile => (if 0 < tile.effect then sq + outcome.value.toNat + 1 else sq + outcome.value.toNat - 1, Option.some tile.owner)).1 ++ List.drop idx (board.getStack sq))) i)) 0 (List.range CamelUp.numSquares)) + List.count c ((board.setStack sq (List.take idx (board.getStack sq))).getStack (match mods[sq + outcome.value.toNat]?.getD Option.none with
        | Option.none => (sq + outcome.value.toNat, Option.none)
        ...
        | Option.none => (sq + outcome.value.toNat, Option.none)
        | Option.some tile => (if 0 < tile.effect then sq + outcome.value.toNat + 1 else sq + outcome.value.toNat - 1, Option.some tile.owner)).1 ++ List.drop idx (board.getStack sq)) := by
          apply_assumption
          generalize_proofs at *; (
