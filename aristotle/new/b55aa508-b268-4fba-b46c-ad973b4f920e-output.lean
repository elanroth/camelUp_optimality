...
This project request had uuid: b55aa508-b268-4fba-b46c-ad973b4f920e
...
- lemma erase_stay_under : ∀ (c : CamelColor) (gs : GameState) (outcome : DieOutcome), ValidState gs → List.count c (gs.diceBag.erase outcome.camel) ≤ 1
...
lemma erase_stay_under : ∀ (c : CamelColor) (gs : GameState) (outcome : DieOutcome), ValidState gs → List.count c (gs.diceBag.erase outcome.camel) ≤ 1 := by
  -- By definition of `ValidState`, the dice bag contains each camel at most once.
  intro c gs outcome h_valid
  have h_count : ∀ c : CamelUp.CamelColor, (gs.diceBag.count c) ≤ 1 := by
    simpa using h_valid.bag_each_once
  simpa using (bag_each_once_erase gs.diceBag outcome.camel h_count c)
...
theorem step_Roll_player_valid (gs : GameState) (outcome : DieOutcome) (gs' : GameState)
  ...
  have h_step' := h_step
  simp [roll_step_simp gs outcome hgo hcontains] at h_step'
  -- By definition of `CamelUp.step`, we know that the new state `gs'` has valid properties.
  apply CamelUp.step_Roll_player_valid gs outcome gs' h_valid h_step
